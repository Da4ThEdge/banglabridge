...
class GetExistingBytesResponse {
  ...
  GetExistingBytesResponse({
    required this.audioStream,
    required this.audioStreamController,
  });
}
...
class SttService {
  ...
  static const double kVadPIsVoiceThreshold = 0.2;
  ...
  bool stopped = false;
  // 🟡 NEW: Store recorded frames for audio saving
  final List<AudioFrame> _frames = [];

  // 🟡 NEW: Expose frames via getter for use in add.dart
  List<AudioFrame> get recordedFrames => _frames;
  ...
  SttService({
    ...
    this.maxSilenceDuration = const Duration(milliseconds: 2000),
  ...
  void _start(StreamController<SttServiceResponse> streamController) async {
    ...
    _frames.clear(); // 🟡 Reuse global _frames instead of local
    ...
    _vadInferenceLoop(vad, _frames, streamController);
  ...
  void _vadInferenceLoop(
    ...
    if (hasBuffer) {
      ...
      Future.delayed(
        const Duration(milliseconds: kMaxVadFrameMs),
        () => _vadInferenceLoop(vad, frames, streamController),
      );
    }
  }
  ...
  Future<void> _processBufferAndVad(
    ...
    while ((index + 1) * frameSizeInBytes <= buffer.length) {
      ...
      final nextVdState = await vad.doInference(
        frameBytes,
        previousState: lastVadState,
      );
  ...
  // 🟡 NEW: Generate WAV bytes from recorded frames
  Uint8List getWavBytes() {
    final List<int> pcm = _frames.expand((f) => f.bytes).toList();
    const int hdrLen = 44;
    final out = Uint8List(hdrLen + pcm.length);
    final bd = ByteData.view(out.buffer);

    bd.setUint32(0, 0x52494646, Endian.big); // "RIFF"
    bd.setUint32(4, pcm.length + hdrLen - 8, Endian.little);
    bd.setUint32(8, 0x57415645, Endian.big); // "WAVE"
    bd.setUint32(12, 0x666d7420, Endian.big); // "fmt "
    bd.setUint32(16, 16, Endian.little); // PCM header length
    bd.setUint16(20, 1, Endian.little); // Audio format
    bd.setUint16(22, kChannels, Endian.little);
    bd.setUint32(24, kSampleRate, Endian.little);
    final byteRate = kSampleRate * kChannels * kBitsPerSample ~/ 8;
    bd.setUint32(28, byteRate, Endian.little);
    bd.setUint16(32, kChannels * kBitsPerSample ~/ 8, Endian.little);
    bd.setUint16(34, kBitsPerSample, Endian.little);
    bd.setUint32(36, 0x64617461, Endian.big); // "data"
    bd.setUint32(40, pcm.length, Endian.little);
    out.setRange(hdrLen, out.length, pcm);
    return out;
  }
}
...
class WhisperSessionManager {
  ...
  List<AudioFrame> getAudioFrames({
    ...
    if (!multipleSegments) {
      final firstVoiceFrameIndex = frames.indexWhere(
        (frame) =>
